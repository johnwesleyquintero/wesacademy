...
export function LoginPage() {
  ...
  const handleFacebookSignIn = async () => {
    ...
    try {
      const { error: facebookSignInError } = await signInWithFacebook();
      if (facebookSignInError) {
        setError(facebookSignInError.message);
      }
    } catch (error: unknown) {
      setError((error as Error).message || 'Failed to sign in with Facebook');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          <Link to="/" className="flex items-center justify-center space-x-2 mb-8">
            ...
            <span className="text-2xl font-bold text-gray-900">WesAcademy</span>
          </Link>
